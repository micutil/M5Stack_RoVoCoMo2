...
bool SimpleBLE::setLocalName(String localName)
{
  local_name = localName;
  adv_data.include_name=(localName.length());
  return _init_gap(local_name.c_str(), &_adv_config);
}

bool SimpleBLE::setIncludeName(bool include_name)
{
  adv_data.include_name=include_name;
  return _init_gap(local_name.c_str(), &_adv_config);
}

bool SimpleBLE::setIncludeTxpower(bool include_txpower)
{
  adv_data.include_txpower=include_txpower;
  return _init_gap(local_name.c_str(), &_adv_config);
}
